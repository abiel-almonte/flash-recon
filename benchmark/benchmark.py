...
from originals import projective_transform as projective_transform_original, induced_flow as induced_flow_original

from new import update_pose as update_pose_new
from new import projective_transform as projective_transform_new, induced_flow as induced_flow_new
...
import warnings
warnings.filterwarnings("ignore", "torch.meshgrid: in an upcoming release") # for induced_flow_original
...
def benchmark_projective_transform():
    ...
    cuda_intrinsics = Intrinsics(intr_o[0].item(), intr_o[1].item(), intr_o[2].item(), intr_o[3].item()).to("cuda")
    ...
    print(f"  Speedup:  {orig_time/cuda_time:.1f}x vs CUDA") #{orig_time/fused_time:.1f}x vs Fused")

    # Correctness check (coords/valid)
    with torch.no_grad():
        coords_o, valid_o = projective_transform_original(orig_poses, orig_depths, orig_intrinsics, ii, jj)
        coords_o = coords_o.squeeze(0)[..., :2]
        valid_o = valid_o.squeeze(0)
        coords_c, valid_c = projective_transform_new(cuda_poses, cuda_depths, cuda_intrinsics, ii, jj, jacobian=False)
        ...
        if both_valid_o.any():
            ...
            max_vo = float('nan'); mean_vo = float('nan'); std_vo = float('nan')

        vmr_fo = (valid_c == valid_o).float().mean().item()

    print(f"  Correctness (Fused vs Orig)   max/mean/std: {max_vo:.2e}/{mean_vo:.2e}/{std_vo:.2e}, valid_match: {vmr_fo:.4f}")
...
def benchmark_induced_flow():
    ...
    orig_time = time_func(lambda: induced_flow_original(orig_poses, orig_depths, orig_intrinsics, ii, jj), n=1000)
    cuda_time = time_func(lambda: induced_flow_new(cuda_poses, cuda_depths, cuda_intrinsics, ii, jj), n=1000)
