...
def compute_reprojection_error(target, weight, poses, disps, intrinsics, ii, jj, n=0):
    ...
    coords, _ = projective_transform(
        poses_window, disps_window, intrinsics, ii_window, jj_window, jacobian=False
    )
    residual = (target_window - coords) * weight_window
    return (residual**2).sum().item()


def compute_depth_alignment_cost(
    disps, mono_disps, scales, shifts, valid_depth_mask, ii, alpha: float, n=0
):
    ...
    scale = torch.where(
        v > 0,
        torch.tensor(10.0, device=d.device, dtype=d.dtype),
...
def run_ba(f_name, data, ref):
    ...
    if is_ba_ss:
        ...
        poses_state = Pose(
            poses_state[:, :3].contiguous(), poses_state[:, 3:].contiguous()
        )
        intr = Intrinsics(
            float(intr[0, 0]),
            float(intr[0, 1]),
        ...
        init_error = compute_depth_alignment_cost(
            disps_state, mono, scales_state, shifts_state, vmask, ii, alpha
        )

        # eta_m is [M, H, W] for M source keyframes - scaling applied internally
        start = time.perf_counter()
        disps_state, scale_shift_params_out = ba_scale_shift(
            target,
            weight,
            eta_m,
            poses_state,
        ...
        new_final_error = compute_depth_alignment_cost(
            disps_state, mono, scales_state, shifts_state, vmask, ii, alpha
        )
        ...
        ref_final_error = compute_depth_alignment_cost(
            ref_final_disps, mono, ref_final_scale, ref_final_shift, vmask, ii, alpha
        )
        ref_latency = ref["latency_s"] * 1000.0

        new_error_reduction = (
            (100 * (init_error - new_final_error) / init_error) if init_error else 0.0
        )
        ref_error_reduction = (
            (100 * (init_error - ref_final_error) / init_error) if init_error else 0.0
        )
        is_good = (
            new_error_reduction > 0
            and (new_error_reduction - ref_error_reduction) > 1.0
        )
        is_bad = (
            ref_error_reduction > 0
            and (ref_error_reduction - new_error_reduction) > 1.0
        )
        ...
        return {
            "f_name": f_name,
            "new": new_error_reduction,
            "ref": ref_error_reduction,
            "bad": is_bad,
            "good": is_good,
            "latency": latency,
        ...
        init_error = compute_reprojection_error(
            target, weight, poses_state, disps_state, intr, ii, jj, t1
        )
        ...
        for _ in range(iters):
            start = time.perf_counter()
            poses_state, disps_state = full_ba(
                target,
                weight,
                eta_m,
                poses_state,
        ...
        new_final_error = compute_reprojection_error(
            target, weight, poses_state, disps_state, intr, ii, jj, t1
        )
        ...
        ref_final_poses = Pose(
            ref_final_poses[:, :3].contiguous(), ref_final_poses[:, 3:].contiguous()
        )
        ref_final_error = compute_reprojection_error(
            target, weight, ref_final_poses, ref_final_disps, intr, ii, jj, t1
        )
...
def main():
    ...
    if results:
        ...
        avg_ref_latency = (
            sum(ref_latencies) / len(ref_latencies) if ref_latencies else float("nan")
        )
        ...
        print(
            f"Avg latency per iter: {avg_latency:.2f} ms | ref: {avg_ref_latency:.2f} ms | speedup: {avg_ref_latency/avg_latency:.2f}x"
        )
        print(
            f"P50 latency per iter: {p50_latency:.2f} ms | ref: {p50_ref_latency:.2f} ms | speedup: {p50_ref_latency/p50_latency:.2f}x"
        )
        print(
            f"P95 latency per iter: {p95_latency:.2f} ms | ref: {p95_ref_latency:.2f} ms | speedup: {p95_ref_latency/p95_latency:.2f}x"
        )
