...
from geometry import Intrinsics, pose_to_matrix, matrix_to_pose
...
def run_case(T=4, H=64, W=64, motion_scale=0.05, iters=50, seed=123, device="cuda"):
    ...
    with torch.inference_mode():
        ...
        for _ in range(iters):
            new_updated_poses, new_updated_disps = ba_new(
                target, weight, eta, poses_cuda, disps, intrinsics, ii, jj
            )
            torch.cuda.synchronize()
    ...
    with torch.inference_mode():
        ...
        for _ in range(iters):
            eta_b = eta_keyframes.unsqueeze(0)
            old_updated_poses, old_updated_disps = ba_old(
