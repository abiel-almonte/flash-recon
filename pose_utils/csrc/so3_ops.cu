...
__global__ void quat_rotate_kernel(const float* q, const float* points, float* result, const int batch_size, const int num_points) {
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= batch_size * num_points) return;
    
    const int batch_idx = idx / num_points;
    const int point_idx = idx % num_points;
    
    const int q_idx = batch_idx * 4;
    const int p_idx = idx * 3;
    ...
    const float px = points[p_idx + 0];
    const float py = points[p_idx + 1];
    const float pz = points[p_idx + 2];
    ...
    result[p_idx + 0] = px + qw * ux + vx;
    result[p_idx + 1] = py + qw * uy + vy;
    result[p_idx + 2] = pz + qw * uz + vz;
}
...
torch::Tensor quat_multiply_cuda(torch::Tensor q1, torch::Tensor q2) {
    ...
    auto result = torch::zeros_like(q1);
...
torch::Tensor quat_rotate_cuda(torch::Tensor q, torch::Tensor points) {
    ...
    auto result = torch::zeros_like(points);
