...
class CorrBlock(nn.Module):
    ...
    def build_pyramid(self, feature1 : torch.Tensor, feature2 : torch.Tensor): #features will be [T, c, h, w] each
    ...
    def forward(self, coords : torch.Tensor): # [T, h, w, 2]
        T, ht, wd, _ = coords.shape
        coords = coords.permute(0, 3, 1, 2).contiguous()

        K = (2 * self.radius + 1) ** 2
        out = torch.empty(T, self.num_levels * K, ht, wd, device=coords.device, dtype=coords.dtype)

        scale = 1
        for i in range(self.num_levels):
            out[:, i*K:(i+1)*K, :, :] = corr_forward(self.pyramid[i], coords / scale, self.radius) # [T, K, h, w]
            scale <<= 1
